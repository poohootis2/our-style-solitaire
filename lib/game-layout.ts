...
export function getGameLayout(width: number, height: number, verticalEdgeInset = 0): GameLayout {
  ...
  const maxCardWidth = isTablet ? 80 : isLandscape ? (height >= 520 ? 72 : 52) : 58;
  ...
  const stackOffset = Math.floor(clamp((verticalRoom - cardWidth * CARD_RATIO) / 6, isLandscape ? 13 : 21, cardWidth * 0.62));
